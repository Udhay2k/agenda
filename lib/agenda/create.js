 ...
 */
module.exports = function(name, data, startDate, endDate, type) {
  debug('Agenda.create(%s, [Object])', name);
  const priority = this._definitions[name] ? this._definitions[name].priority : 0;
  const job = new Job({ startRunsAt: startDate, noMoreAt: endDate, IntervalType: type, name, data, type: 'normal', priority, agenda: this });
  return job;
};
