 ...
 */
module.exports = function (when, names, data, startDate, endDate, type) {
  const self = this;
   ...
   */
  const createJob = async (when, name, data, startDate, endDate, type) => {
   ...
   */
  const createJobs = async (when, names, data, startDate, endDate, type) => {
    try {
      const jobs = await Promise.all(names.map(name => createJob(when, name, data, startDate, endDate, type)));
  ...
  if (typeof names === 'string' || names instanceof String) {
    ...
    return createJob(when, names, data, startDate, endDate, type);
  }

  if (Array.isArray(names)) {
    debug('Agenda.schedule(%s, %O, [%O])', when, names);
    return createJobs(when, names, data, startDate, endDate, type);
  }
};
